...
public class Funcionario17 {
    ...
    public Funcionario17(String nome, String cpf, int idade, String departamento) {
        setNome(nome);
        setCpf(cpf);
        setIdade(idade);
    ...
    private void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("[\\p{L}]+( [\\p{L}]+)+")){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario17.ERRO_NOME.getDescricao());
        }
    }

    private void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() !=11 || !cpf.matches("\\d{11}") || !isCpfValido(cpf) ){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario17.ERRO_CPF.getDescricao());
        }
    }
    ...
    private void validacaoIdade(int idade){
        if (idade < IDADE_MINIMA){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario17.ERRO_IDADE.getDescricao());
        }
    }
    ...
    private void validacaoDepartamento(String departamento){
        if (departamento == null || departamento.isEmpty() || !departamento.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario17.ERRO_DEPARTAMENTO.getDescricao());
        }
    }
    ...
    public enum MensagensValidacaoFuncionario17{
        ERRO_NOME("Campo nome não pode ser vazio ou conte caracteres."),
        ...
        ERRO_SALARIO_GESTAO("Salário de cargo de gestão não pode ser menor que R$3.500"),
        ...
        ERRO_IDADE("Idade não pode ser menor que 18 anos.");

        private final String descricao;

        MensagensValidacaoFuncionario17(String descricao){
