...
public abstract class Veiculo34 {
    protected String placa;
    protected int anoFabricacao;
    protected String cor;
    protected double valorMercado;
    protected List<Double> historicoPagamentoIPVA = new ArrayList<>();

    public Veiculo34(String placa, int anoFabricacao, String cor, double valorMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new PlacaInvalidaException34();
        }
    }
    ...
    public static void validacaoPlacaDuplicada(String placa){
        if (placasCadastradas.contains(placa)){
            throw new PlacaDuplicadaException34();
        }
    }

    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < AnoFabricacaoException34.MENOR_ANO_FABRICACAO || anoFabricacao > AnoFabricacaoException34.MAIOR_ANO_FABRICACAO){
            throw new AnoFabricacaoException34();
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new CorInvalidaException34();
        }
    }
    ...
    public static void validacaoValorMercado(double valorMercado){
        if (valorMercado < ValorMercadoInvalidoException34.MENOR_VALOR_MERCADO){
            throw new ValorMercadoInvalidoException34();
        }
    }
    ...
    public void registrarPagamentoIPVA(){
        ...
        historicoPagamentoIPVA.add(ipva);
