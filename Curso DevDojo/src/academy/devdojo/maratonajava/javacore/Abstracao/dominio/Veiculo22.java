...
public abstract class Veiculo22 {
    ...
    public Veiculo22(String placa, int anoFabricacao, String cor, double valorDeMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo22.PLACA.getDescricao());
        }
    }
    ...
    public static void validacaoPlacasDuplicadas(String placa){
        if (placasCadastradas.contains(placa)){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo22.PLACA_DUPLICADA.getDescricao());
        }
    }
    ...
    public static void validacaoAnoFabricao(int anoFabricacao){
        if (anoFabricacao < MENOR_ANO_FABRICACAO || anoFabricacao > MAIOR_ANO_FABRICACAO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo22.ANO_FABRICACAO.getDescricaoFormatada(MENOR_ANO_FABRICACAO,MAIOR_ANO_FABRICACAO));
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo22.COR.getDescricao());
        }
    }
    ...
    public static void validacaoValorMercado(double valorDeMercado){
        if (valorDeMercado < MENOR_VALOR_MERCADO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo22.VALOR_MERCADO.getDescricaoFromatada(MENOR_VALOR_MERCADO));
        }
    }
    ...
    public void setAnoFabricacao(int anoFabricacao) {
        validacaoAnoFabricao(anoFabricacao);
    ...
    public void setValorDeMercado(double valorDeMercado) {
        validacaoValorMercado(valorDeMercado);
    ...
    public void mostrarHistoricoPagamentoIPVA(){
        if (historicoIPVA.isEmpty()){
            ...
            System.out.println("Histórico de pagamento IPVA - placa:"+placa);
    ...
    public enum MensagensValidacaoVeiculo22{
        ...
        MensagensValidacaoVeiculo22(String descricao) {
        ...
        public String getDescricaoFromatada(double... valores){
            return String.format(descricao, Arrays.stream(valores).boxed().toArray());
        }
    }
    ...
    public String toString(){
        return String.format("Placa: %s |Ano de fabricação: %s |Cor: %s |Valor de mercado:R$%.2f.",
