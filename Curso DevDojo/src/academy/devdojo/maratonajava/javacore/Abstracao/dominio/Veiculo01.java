...
public abstract class Veiculo01 {
    ...
    public Veiculo01(String placa, int anoFabricacao, String cor,double valorDeMercado) {
        setPlaca(placa);
    ...
    private static final Map<Integer, Double> ALIQUOTAS = new TreeMap<>(Collections.reverseOrder());
    static {
    ...
    private boolean isVeiculoComMais20Anos(){
    ...
    protected double getAliquotaIPVA(){
        ...
        for (Integer anoLimite : ALIQUOTAS.keySet()){
            if (anoFabricacao >= anoLimite){
                return ALIQUOTAS.get(anoLimite);
            }
        }
        return 0.0;
    }
    ...
    public static final int ANO_MINIMO_FABRICACAO = 1800;
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo01.ERRO_PLACA.getDescricao());
        }
    }
    ...
    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < ANO_MINIMO_FABRICACAO || anoFabricacao > ANO_MAXIMO_FABRICACAO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo01.ERRO_ANO_FABRICACAO.getDescricaoFormatada(ANO_MINIMO_FABRICACAO,ANO_MAXIMO_FABRICACAO));
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo01.ERRO_COR.getDescricao());
        }
    }
    ...
    public static void validacaoValorDeMercado(double valorDeMercado){
        if (valorDeMercado < VALOR_MINIMO_MERCADO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo01.ERRO_VALOR_MERCADO.getDescricaoFormatada(VALOR_MINIMO_MERCADO));
        }
    }
    ...
    public void registrarPagamentoIPVA(){
        ...
        System.out.println("Pagamento IPVA registrado:R$ "+ipva);
        mostrarHistoricoIPVA();
    }

    public void mostrarHistoricoIPVA(){
        if (historicoIPVA.isEmpty()){
            System.out.println(MensagensValidacaoVeiculo01.HISTORICO_IPVA_VAZIO.getDescricao());
        }else {
    ...
    public List<Double> getHistoticoIPVA() {
    ...
    public enum MensagensValidacaoVeiculo01{
        ERRO_PLACA("Para validar placas, precisam ter o formato AAA-0000 ou AAA0A00."),
        ERRO_ANO_FABRICACAO("Ano de fabricação não pode ser menor que %d ou maior que %d."),
        HISTORICO_IPVA_VAZIO("Nenhum histórico de IPVA registrado."),
        ERRO_VALOR_MERCADO("Valor de mercado não pode ser menor que R$%.2f."),
        ERRO_COR("Campo cor não pode conter caracteres ou ficar vazio.");

        public final String descricao;

        MensagensValidacaoVeiculo01(String descricao){
    ...
    public String toString(){
        return String.format("Placa: %s |Ano de fabricação: %d |Cor: %s",placa,anoFabricacao,cor);
    }
