...
public class Cliente03 {
    ...
    public Cliente03(String nome, String cpf) {
        setNome(nome);
        setCpf(cpf);
    }
    ...
    public void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("^[\\p{L}]+( [\\p{L}]+)+$")){
            throw new IllegalArgumentException(MensagensErro.ERRO_NOME.getDescricao());
        }
    }
    ...
    public void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() != 11 || !cpf.matches("\\d{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagensErro.ERRO_CPF.getDescricao());
        }
    }
    ...
    public enum MensagensErro{
        ...
        ERRO_CONTA("Número da conta precisa ter mínimo de 6 digitos."),
        ...
        MensagensErro(String descricao){
