...
public abstract class Veiculo20 {
    ...
    public Veiculo20(String placa, int anoFabricacao, String cor, double valorDeMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlacaDuplicada(String placa){
        if (placasCadastradas.contains(placa)){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo20.PLACA_DUPLICADA.getDescricao());
        }
    }

    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo20.PLACA.getDescricao());
        }
    }
    ...
    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < MENOR_ANO_FABRICACAO || anoFabricacao > MAIOR_ANO_FABRICACAO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo20.ANO_FABRICACAO.getDescricao());
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("\\p{L}+( \\p{L}+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo20.COR.getDescricao());
        }
    }
    ...
    public static void validacaoValorDeMercado(double valorDeMercado){
        if (valorDeMercado < MENOR_VALOR_MERCADO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo20.VALOR_DE_MERCADO.getDescricaoFormatada(MENOR_VALOR_MERCADO));
        }
    }
    ...
    public void registrarPagamentoIPVA(){
        ...
        System.out.println("Pagamento registrado no valor R$"+df.format(ipva)+" para a placa:"+getPlaca());
    }
    ...
    public enum MensagensValidacaoVeiculo20{
        ...
        MensagensValidacaoVeiculo20(String descricao) {
        ...
        private String getDescricaoFormatada(double... valores){
