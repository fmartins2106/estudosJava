...
public abstract class Veiculo43 {
    ...
    public Veiculo43(String placa, int anoFabricacao, String cor, double valorMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new PlacaInvalidaException43();
        }
    }
    ...
    public static void validacaoPlacaDuplicada(String placa){
        if (placasCadastradas.contains(placa)){
            throw new PlacaDuplicada43();
        }
    }

    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < AnoFabricacaoException43.MENOR_ANO_FABRICACAO || anoFabricacao > AnoFabricacaoException43.MAIOR_ANO_FABRICACAO){
            throw new AnoFabricacaoException43();
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new CorInvalidaException43();
        }
    }
    ...
    public static void validacaoValorMercado(double valorMercado){
        if (valorMercado < ValorMercadoException43.MENOR_VALOR_MERCADO){
            throw new ValorMercadoException43();
        }
    }
    ...
    public void mostrarHistoricoIPVA(){
        ...
        System.out.println("Histórico IPVA:"+placa);
