...
import academy.devdojo.maratonajava.javacore.datas.execessoes.DataHoraInvalida13;
...
public abstract class EventoBase13 implements Agendavel13{
    protected String nomeEvento;
    protected String localEvento;
    protected ZonedDateTime dataHora;

    public EventoBase13(String nomeEvento, String localEvento, ZonedDateTime dataHora) {
    ...
    public static void validacaoNomeEvento(String nomeEvento){
        if (nomeEvento == null || nomeEvento.isEmpty() || !nomeEvento.matches("^[\\p{L}0-9]+( [\\p{L}0-9]+)*$")){
            throw new NomeEventoBase13();
        }
    }

    public static void validacaoLocalEvento(String localEvento){
        if (localEvento == null || localEvento.isEmpty() || !localEvento.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new LocalEventoBase13();
        }
    }
    ...
    public static void validacaoFormatoDataHora(ZonedDateTime  dataHora){
        if (dataHora == null || dataHora.isBefore(ZonedDateTime.now(ZoneId.of("America/Sao_Paulo")))){
            throw new DataHoraInvalida13();
        }
    }
    ...
    public void setDataHora(ZonedDateTime dataHora) {
        validacaoFormatoDataHora(dataHora);
