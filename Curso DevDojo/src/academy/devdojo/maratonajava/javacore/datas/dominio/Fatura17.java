...
public class Fatura17 {
    private String numero;
    private double valor;
    private LocalDate vencimento;
    private LocalDateTime dataHoraCadastro;

    public Fatura17(String numero, double valor, LocalDate vencimento) {
        setNumero(numero);
        setValor(valor);
    ...
    public static void validacaoNumeroFatura(String numero){
        if (numero == null || numero.isEmpty() || !numero.matches("^[a-zA-Z0-9]+$")){
            throw new NumeroFaturaException17();
        }
    }

    public static void validacaoValorFatura(double valor){
        if (valor < ValorFaturaException17.MENOR_VALOR_FATURA){
            throw new ValorFaturaException17();
        }
    }

    public static void validacaoDataVencimento(LocalDate vencimento){
        if (vencimento == null /*|| vencimento.isBefore(LocalDate.now())*/){
            throw new DataVencimentoFaturaException17();
        }
    }
    ...
    public void setVencimento(LocalDate vencimento) {
        ...
        this.vencimento = Feriados17.ajustarParaProximoDiaUtil(vencimento);
    }
