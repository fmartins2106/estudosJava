...
public class Pessoa21 {
    ...
    public Pessoa21(String nome, String cpf, int idade) {
        setNome(nome);
        setCpf(cpf);
        setIdade(idade);
    }
    ...
    public static void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("^[\\p{L}]+( [\\p{L}]+)+$")){
            throw new IllegalArgumentException(MensagensValidacaoPessoa21.ERRO_NOME.getDescricao());
        }
    }

    public static void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() != 11 || !cpf.matches("\\d{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagensValidacaoPessoa21.ERRO_CPF.getDescricao());
        }
    }
    ...
    public static void validacaoIdade(int idade){
        if (idade < IDADE_MINIMA){
            throw new IllegalArgumentException(MensagensValidacaoPessoa21.ERRO_IDADE.getDescricaoFormatada(IDADE_MINIMA));
        }
    }
    ...
    public enum MensagensValidacaoPessoa21{
        ...
        ERRO_MATRICULA_DUPLICADA("Matrículo já cadastrada, tente outro número."),
        ERRO_IDADE("Idade não pode ser menor que %d");

        private final String descricao;

        MensagensValidacaoPessoa21(String descricao){
