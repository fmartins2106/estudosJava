...
public class Professor14 extends Pessoa14{
    private String disciplina;
    private double salario;

    public Professor14(String nome, String cpf, int idade, String disciplina, double salario) {
    ...
    private void validacaoSalarioProf(double salario){
        if (salario < SALARIO_MINIMO_PROF){
            throw new IllegalArgumentException(MensagensValidacoes.ERRO_SALARIO_MINIMO_PROF.getDescricao());
        }
    }

    private void validacaoDisciplina(String disciplina){
        if (disciplina == null || disciplina.isEmpty() || !disciplina.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MensagensValidacoes.ERRO_DISCIPLINA.getDescricao());
        }
    }
