...
public class Funcionario20 {
    ...
    public Funcionario20(String nome, String cpf, int idade, String departamento) {
        setNome(nome);
        setCpf(cpf);
        setIdade(idade);
    ...
    public static void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("[\\p{L}]+( [\\p{L}]+)+$")){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario20.ERRO_NOME.getDescricao());
        }
    }

    public static void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() != 11 || !cpf.matches("\\d+{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario20.ERRO_CPF.getDescricao());
        }
    }
    ...
    public static void validacaoIdade(int idade){
        if (idade < IDADE_MINIMA){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario20.ERRO_IDADE.getDescricaoFormatada(IDADE_MINIMA));
        }
    }
    ...
    public static void validacaoDepartamento(String departamento){
        if (departamento == null || departamento.isEmpty() || !departamento.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoFuncionario20.ERRO_DEPARTAMENTO.getDescricao());
        }
    }
    ...
    public enum MensagensValidacaoFuncionario20{
        ERRO_NOME("Digite nome completo. Campo nome não pode ser vazio ou conter caracteres."),
        ERRO_CPF("CPF inválido."),
        ERRO_CARGO_GESTAO("Campo cargo de gestão, não pode ser vazio ou conter caracteres."),
        ERRO_SALARIO_CARGO_GESTO("Salário de cargo gestão não pode ser menor que R$%.2f."),
        ERRO_CARGO_GERAL("Campo cargo não pode ser vazio ou conter caracteres."),
        ERRO_IDADE("Idade não pode ser menor que %d anos."),
        ...
        MensagensValidacaoFuncionario20(String descricao){
