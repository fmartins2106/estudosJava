...
public class Pessoa25 {
    private String nome;
    private String cpf;
    private int idade;

    public Pessoa25(String nome, String cpf, int idade) {
        setNome(nome);
        setCpf(cpf);
        setIdade(idade);
    }
    ...
    public static void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("^\\p{L}+( \\p{L}+)+$")){
            throw new IllegalArgumentException(MensagensValidacaoPessoa25.ERRO_NOME.getDescricao());
        }
    }
    ...
    public static void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() !=11 || !cpf.matches("\\d{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagensValidacaoPessoa25.ERRO_CPF.getDescricao());
        }
    }
    ...
    public static void validacaoIdade(int idade){
        if (idade < MENOR_IDADE){
            throw new IllegalArgumentException(MensagensValidacaoPessoa25.ERRO_IDADE.getDescricaoFormatada(MENOR_IDADE));
        }
    }
    ...
    public enum MensagensValidacaoPessoa25{
        ...
        MensagensValidacaoPessoa25(String descricao){
