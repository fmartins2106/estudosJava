...
public class Gerente13 extends Funcionario13{
    private String cargoGestao;
    private double salario;

    public Gerente13(String nome, String cpf, int idade, String departamento, String cargoGestao, double salario) {
        super(nome, cpf, idade, departamento);
        setCargoGestao(cargoGestao);
        setSalario(salario);
    }

    public static final String MENSAGEM_ERR0_CARGO_GESTAO = "Campo não pode ser vazio ou conter caracteres.";
    public static final String MENSAGEM_ERRO_SALARIO_GESTAO = "Salário não pode ser menor que R$4000.";
    public static final int SALARIO_MINIMO_GESTAO = 4000;

    private void validacaoCargoGestao(String cargoGestao){
        if (cargoGestao == null || cargoGestao.isEmpty() || !cargoGestao.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MENSAGEM_ERR0_CARGO_GESTAO);
        }
    }
    ...
    public static String validandoCargoGestao(Scanner scanner){
        while (true){
            ...
            if (cargoGestao.isEmpty() || !cargoGestao.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
                System.out.println(MENSAGEM_ERR0_CARGO_GESTAO);
                continue;
            }
    ...
    public static Gerente13 validandoDadosGestao(Scanner scanner,String nome, String cpf, int idade, String departamento){
        String cargoGestao = validandoCargoGestao(scanner);
        double salario = validandoSalario(scanner);
        return new Gerente13(nome,cpf,idade,departamento, cargoGestao,salario);
    }
