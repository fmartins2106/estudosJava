...
public class Pessoa18 {
    ...
    public Pessoa18(String nome, String cpf, int idade) {
        setNome(nome);
        setCpf(cpf);
        setIdade(idade);
    }
    ...
    private void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("^[\\p{L}]+( [\\p{L}]+)+$")){
            throw new IllegalArgumentException(MensagensValidacaoPessoas18.ERRO_NOME.getDescricao());
        }
    }

    private void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() != 11 || !cpf.matches("\\d{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagensValidacaoPessoas18.ERRO_CPF.getDescricao());
        }
    }
    ...
    private void validacaoIdade(int idade){
        if (idade < IDADE_MINIMA){
            throw new IllegalArgumentException(MensagensValidacaoPessoas18.ERRO_IDADE.getDescricao());
        }
    }
    ...
    public enum MensagensValidacaoPessoas18{
        ...
        ERRO_MATRICULA_DUPLICADA("Erro. Matricula já existe."),
        ERRO_SALARIO_PROFESSOR("Salário de professor não pode ser menor que R$3.500"),
        ...
        MensagensValidacaoPessoas18(String descricao){
