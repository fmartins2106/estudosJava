...
public class Professor16 extends Pessoa16{
    private String disciplina;
    private double salario;

    public Professor16(String nome, String cpf, int idade, String disciplina, double salario) {
    ...
    public void validacaoDisciplina(String disciplina){
        if (disciplina == null || disciplina.isEmpty() || !disciplina.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoPessoas16.ERRO_DISCIPLINA.getDescricao());
        }
    }

    public void validacaoSalarioProfessor(double salario){
        if (salario < SALARIO_MINIMO_PROFESSOR){
            throw new IllegalArgumentException(MensagensValidacaoPessoas16.ERRO_SALARIO_PROF.getDescricao());
        }
    }
