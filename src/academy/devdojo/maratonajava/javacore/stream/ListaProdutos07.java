...
public class ListaProdutos07 {
    ...
    public Map<Categoria03,List<ProdutoStream03>> agruparPorCategoria(List<ProdutoStream03> produtos){
        return produtos.stream()
                .collect(Collectors.groupingBy(ProdutoStream03::getCategoria03));
    }

    public Map<Categoria03,Map<Promocao03,List<ProdutoStream03>>> agruparPorCategoriaPromocao(List<ProdutoStream03> produtos){
        return produtos.stream()
                .collect(Collectors.groupingBy(ProdutoStream03::getCategoria03
                ,Collectors.groupingBy(produtoStream03
                                -> produtoStream03.getPreco() < 5 ? Promocao03.PROMOCAO : Promocao03.PRECO_NORMAL)));
    }

    public Map<Promocao03,List<ProdutoStream03>> agruparPorPromocao(List<ProdutoStream03> produto){
        return produto.stream()
                .collect(Collectors.groupingBy(produtoStream03
                        -> produtoStream03.getPreco() < 5 ? Promocao03.PROMOCAO : Promocao03.PRECO_NORMAL));
    }

    public Map<Categoria03,Long> contarProdutosPorCategoria(List<ProdutoStream03> produtos){
        return produtos.stream()
                .collect(Collectors.groupingBy(ProdutoStream03::getCategoria03
                ,Collectors.counting()));
    }

    public Map<Categoria03, Optional<ProdutoStream03>> produtoMaisCaroCategoria(List<ProdutoStream03> produto){
        return produto.stream()
                .collect(Collectors.groupingBy(ProdutoStream03::getCategoria03
                ,Collectors.maxBy(Comparator.comparing(ProdutoStream03::getPreco))));
    }

    public Map<Categoria03,DoubleSummaryStatistics> mostrarEstatisticasPorCategoria(List<ProdutoStream03> produtos){
        return produtos.stream()
                .collect(Collectors.groupingBy(ProdutoStream03::getCategoria03,
                        Collectors.summarizingDouble(ProdutoStream03::getPreco)));
    }

    public Map<Categoria03,Map<Promocao03,List<ProdutoStream03>>> agruparPorMetodoAuxiliar(List<ProdutoStream03> produtos){
        return produtos.stream()
                .collect(Collectors.groupingBy(ProdutoStream03::getCategoria03,
                        Collectors.groupingBy(ListaProdutos07::getPromocao)));
    }
