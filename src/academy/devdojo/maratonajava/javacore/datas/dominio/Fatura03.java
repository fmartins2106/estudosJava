...
import academy.devdojo.maratonajava.javacore.datas.execessoes.NumeroFaturaDuplicadaException03;
...
public class Fatura03 {
    private String numero;
    private double valor;
    private LocalDate vencimento;
    private LocalDateTime dataHoraCadastro;

    public Fatura03(String numero, double valor, LocalDate vencimento) {
        setNumero(numero);
        setValor(valor);
    ...
    public static void validacaoNumeroFatura(String numero){
        if (numero == null || numero.isEmpty() || !numero.matches("^[a-zA-Z0-9/._-]+$")){
            throw new NumeroFaturaException03();
        }
        if (faturasCadastradas.contains(numero)){
            throw new NumeroFaturaDuplicadaException03();
        }
    }

    public static void validacaoValorFatura(double valor){
        if (valor < ValorFaturaException03.MENOR_PRECO){
            throw new ValorFaturaException03();
        }
    }

    public static void validacaoVencimento(LocalDate vencimento){
        if (vencimento == null){
            throw new DataVencimentoFaturaException03();
        }
    }
    ...
    public void setVencimento(LocalDate vencimento) {
        validacaoVencimento(vencimento);
        this.vencimento = Feriados03.ajustarParaProximoDiaUtil(vencimento);
