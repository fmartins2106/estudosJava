...
public class Feriados22 {
    ...
    public static boolean eDiaUtil(LocalDate date){
        Set<LocalDate> ferioados = gerarFeriados(date.getYear());
        DayOfWeek diaSemana = date.getDayOfWeek();
        return diaSemana != DayOfWeek.SUNDAY &&
                diaSemana != DayOfWeek.SATURDAY &&
                !ferioados.contains(date);
    }

    public static LocalDate ajudarProximoDiaUtil(LocalDate date){
        if (eDiaUtil(date)){
