...
public class Fatura35 {
    private String numero;
    private double valor;
    private LocalDate vencimento;
    private LocalDateTime dataHoraCadastro;

    public Fatura35(String numero, double valor, LocalDate vencimento) {
        this.numero = numero;
        this.valor = valor;
    ...
    public static void validacaoNumeroFatura(String numero){
        if (numero == null || numero.isEmpty() || !numero.matches("^[\\p{L}0-9/_.-]+$")){
            throw new NumeroFaturaException35();
        }
    }

    public static Set<String> faturtasDuplicadas = new HashSet<>();

    public static void validacaoFaturaDuplicada(String numero){
        if (faturtasDuplicadas.contains(numero)){
            throw new NumeroFaturaDuplicadaException35();
        }
    }

    public static void validacaoValorFatura(double valor){
        if (valor < ValorFaturaException35.MENOR_VALOR_FATURA){
            throw new ValorFaturaException35();
        }
    }

    public static void validacaoDataVencimento(LocalDate vencimento){
        if (vencimento ==  null /*|| vencimento.isBefore(LocalDate.now())*/){
            throw new DataVencimentoFaturaException35();
        }
    }
