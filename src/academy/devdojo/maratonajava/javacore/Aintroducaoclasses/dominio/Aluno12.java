...
public class Aluno12 implements Cloneable{
    private String nome;
    ...
    public Aluno12(String nome, double nota1, double nota2){
        setNome(nome);
        setNota1(nota1);
        setNota2(nota2);
    }
    ...
    private void validandoNota(double nota){
        if (nota < MIN_NOTA || nota > MAX_NOTA){
            throw new IllegalArgumentException("Nota precisa estar entre 0 e 10.");
        }
    }
    ...
    private void definindoSituacao(){
        if (media >=7){
    ...
    @Override
    public Aluno12 clone(){
        try {
            return (Aluno12) super.clone();
