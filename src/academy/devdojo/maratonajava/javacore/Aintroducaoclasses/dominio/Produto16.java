...
public class Produto16 {
    ...
    public Produto16(int codigo, String nome, String categoria, double valor, int quantidade, int estoqueMinimo){
        setCodigo(codigo);
        setNome(nome);
    ...
    public void setNome(String nome) {
        if (nome == null || nome.isEmpty() || !nome.matches("^[\\p{L}]+( [\\p{L}]+)+$")){
            throw new IllegalArgumentException("Digite seu nome completo sem adição de caracateres.");
        }
    ...
    public void setEstoqueMinimo(int estoqueMinimo) {
        if (estoqueMinimo < 50){
            System.out.println("Estoque minimo não pode ser menor que 50.");
        }
