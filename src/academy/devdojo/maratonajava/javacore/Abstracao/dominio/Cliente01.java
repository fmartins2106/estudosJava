...
public class Cliente01 {
    ...
    public Cliente01(String nome, String cpf) {
        setNome(nome);
        setCpf(cpf);
    }
    ...
    public void validacaoCpf(String cpf){
        if (cpf == null || cpf.length() !=11 || !cpf.matches("\\d{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagemValidacao.ERRO_CPF.getDescricao());
        }
    }
    ...
    public enum MensagemValidacao{
        ...
        ERRO_NUMERO_CONTA("Conta precisa positivo e ter mais de 6 digitos."),
        ERRO_ENCERRAR_CONTA("Conta encerrada."),
        ERRO_CONTA_BLOQUEADA("Conta bloqueada"),
        ERRO_CONTA_INATIVA("Operação negada, conta não esta ativa."),
