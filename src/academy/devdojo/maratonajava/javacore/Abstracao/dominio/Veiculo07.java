...
public abstract class Veiculo07 {
    ...
    public Veiculo07(String placa, int anoFabricacao, String cor, double valorDeMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo07.ERRO_PLACA.getDescricao());
        }
    }
    ...
    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < MENOR_ANO_FABRICACAO || anoFabricacao > MAIOR_ANO_FABRICACAO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo07.ERRO_ANO_FABRICACAO.getDescricaoFormatada(MENOR_ANO_FABRICACAO,MAIOR_ANO_FABRICACAO));
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^[\\p{L}]+( [\\p{L}]+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo07.ERRO_COR.getDescricao());
        }
    }
    ...
    public static void validacaoValorDeMercado(double valorDeMercado){
        if (valorDeMercado < MENOR_VALOR_MERCADO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo07.ERRO_VALOR_MERCADO.getDescricaoFormatada(MENOR_VALOR_MERCADO));
        }
    }
    ...
    public void mostrarHistoricoIPVA(){
        if (historicoIPVA.isEmpty()){
            System.out.println(MensagensValidacaoVeiculo07.LISTA_HISTORICO_IPVA_VAZIO.getDescricao());
        }else {
            System.out.println(MensagensValidacaoVeiculo07.HISTORICO_IPVA_PLACA.getDescricaoFormatada(getPlaca()));
    ...
    public void registrarPagamentoIPVA(){
        ...
        System.out.println(MensagensValidacaoVeiculo07.PAGAMENTO_IPVA_REGISTRADO.getDescricaoFormatada(IPVA));
    ...
    public enum MensagensValidacaoVeiculo07{
        ERRO_PLACA("Placa deve ter o formato AAA000 ou AAA0A00."),
        ERRO_ANO_FABRICACAO("Ano de fabricação não pode ser menor que %d ou maior que %d."),
        ERRO_COR("Campor cor não pode ser vazio ou conter caracteres."),
        LISTA_HISTORICO_IPVA_VAZIO("Lista vazia."),
        PAGAMENTO_IPVA_REGISTRADO("Pagamento de IPVA registrado: %.2f"),
        HISTORICO_IPVA_PLACA("Histórico IPVA para a placa: %s"),
        ERRO_VALOR_MERCADO("Valor de mercado não pode ser menor que %.2f.");

        private final String descricao;

        MensagensValidacaoVeiculo07(String descricao){
    ...
    public String toString(){
        return String.format("Placa: %s |Ano fabricação: %d |Cor: %s |Valor de mercado: %.2f",placa,anoFabricacao,cor,valorDeMercado);
    }
}
