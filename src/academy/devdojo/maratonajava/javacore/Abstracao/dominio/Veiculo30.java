...
public abstract class Veiculo30 {
    ...
    public Veiculo30(String placa, int anoFabricacao, String cor, double valorMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo30.PLACA.getDescricao());
        }
    }
    ...
    public static void validandoPlacasDuplicadas(String placa){
        if (placasCadastradas.contains(placa)){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo30.PLACA_DUPLICADA.getDescricao());
        }
    }

    private static final int MENOR_ANO = 1900;
    private static final int MAIOR_ANO = Year.now().getValue();

    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < MENOR_ANO || anoFabricacao > MAIOR_ANO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo30.ANO_FABRICACAO.getDescricaoFormatada(MENOR_ANO,MAIOR_ANO));
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo30.COR.getDescricao());
        }
    }

    public static String formatoString(String nome){
        String[] palavras = nome.toLowerCase().split("\\s+");
        StringBuilder nomeFormatada = new StringBuilder();
        for (String palavra : palavras) {
            nomeFormatada.append(palavra.substring(0,1).toUpperCase())
                    .append(palavra.substring(1))
                    .append(" ");
        }
        return nomeFormatada.toString().trim();
    }
    ...
    public static void validacaoValorMercado(double valorMercado){
        if (valorMercado < MENOR_VALOR_MERCADO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo30.VALOR_MERCADO.getDescricaoFormatada(MENOR_VALOR_MERCADO));
        }
    }
    ...
    public void registrarPagamentoIPVA(){
        ...
        System.out.println("Pagamento de IPVA no valor de R$"+df.format(ipva)+" registrado para placa:"+df.format(placa));
    }
    ...
    public enum MensagensValidacaoVeiculo30{
        PLACA("Digite uma placa no formato AAAOOOO ou AAA0A00."),
        ...
        MensagensValidacaoVeiculo30(String descricao) {
