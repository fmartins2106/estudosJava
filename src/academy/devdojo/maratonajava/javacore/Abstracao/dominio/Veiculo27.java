...
public abstract class Veiculo27 {
    ...
    public Veiculo27(String placa, int anoFabricacao, String cor, double valorMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo27.PLACA.getDescricao());
        }
    }
    ...
    public static void validacaoPlacaDuplicada(String placa){
        if (placasCadastradas.contains(placa)){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo27.PLACA_DUPLIACA.getDescricao());
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo27.COR.getDescricao());
        }
    }
    ...
    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < MENOR_ANO_FABRICACAO || anoFabricacao > MAIOR_ANO_FABRICACAO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo27.ANO_FABRICACAO.getDescricaoFormatada(MENOR_ANO_FABRICACAO,MAIOR_ANO_FABRICACAO));
        }
    }

    private static final double MENOR_VALOR = 0;

    public static void validacaoValorMercado(double valorMercado){
        if (valorMercado < MENOR_VALOR){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo27.VALOR_MERCADO.getDescricaoFormatada(MENOR_VALOR));
        }
    }
    ...
    public void registrarPagamentoIPVA(){
        ...
        System.out.println("Pagamento de IPVA nova valor de R$"+df.format(ipva)+" registrado com sucesso para placa:"+placa);
    }
    ...
    public enum MensagensValidacaoVeiculo27{
        ...
        MensagensValidacaoVeiculo27(String descricao) {
