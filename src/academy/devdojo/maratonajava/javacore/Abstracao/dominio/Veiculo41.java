...
public abstract class Veiculo41 {
    ...
    public Veiculo41(String placa, int anoFabricacao, String cor, double valorMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new PlacaInvalidaException41();
        }
    }
    ...
    public static void validacaoPlacaDuplicada(String placa){
        if (placasCadastradas.contains(placa)){
            throw new PlacaDuplicadaException41();
        }
    }

    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < AnoFabricacaoException41.MENOR_ANO_FABRICACAO || anoFabricacao > AnoFabricacaoException41.MAIOR_ANO_FABRICACAO){
            throw new AnoFabricacaoException41();
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new CorInvalidaException41();
        }
    }

    public static String formatoCor(String nome){
    ...
    public static void validacaoValorMercado(double valorMercado){
        if (valorMercado < ValorMercadoInvalidoException41.MENOR_VALOR_MERCADO){
            throw new ValorMercadoInvalidoException41();
        }
    }
