...
public abstract class Veiculo12 {
    ...
    public Veiculo12(String placa, int anoFabricacao, String cor, double valorDeMercado) {
        setPlaca(placa);
    ...
    public static void validacaoPlaca(String placa){
        if (placa == null || placa.isEmpty() || !placa.matches("^[A-Z]{3}[0-9]{4}$|^[A-Z]{3}[0-9][A-Z][0-9]{2}$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo12.ERRO_PLACA.getDescricao());
        }
        if (placasCadastradas.contains(placa)){
            System.out.println(MensagensValidacaoVeiculo12.PLACA_DUPLICADA.getDescricao());
        }
    }
    ...
    public static void validacaoAnoFabricacao(int anoFabricacao){
        if (anoFabricacao < MENOR_ANO_FABRICACAO || anoFabricacao > MAIOR_ANO_FABRICACAO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo12.ERRO_ANO_FABRICACAO.getDescricaoFormatada(MENOR_ANO_FABRICACAO,MAIOR_ANO_FABRICACAO));
        }
    }

    public static void validacaoCor(String cor){
        if (cor == null || cor.isEmpty() || !cor.matches("^\\p{L}+( \\p{L}+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo12.ERRO_COR.getDescricao());
        }
    }
    ...
    public static void validacaoValorDeMercado(double valorDeMercado){
        if (valorDeMercado < MENOR_VALOR_DE_MERCADO){
            throw new IllegalArgumentException(MensagensValidacaoVeiculo12.ERRO_VALOR_DE_MERCADO.getDescricaoFormatada(MENOR_VALOR_DE_MERCADO));
        }
    }
    ...
    public void registrarPagamentoIPVA(){
        ...
        System.out.println("Pagamento de IPVA:R$"+IPVA+" | Registrado para placa:"+getPlaca());
    }
    ...
    public void mostrarHistoricoIPVA(){
        if (historicoIPVA.isEmpty()){
            ...
            System.out.println("Histórico placa:"+getPlaca());
    ...
    public enum MensagensValidacaoVeiculo12{
        ERRO_PLACA("Digite placa no formato AAA0000 ou AAA0A00."),
        ERRO_ANO_FABRICACAO("Ano de fabricação deve ser maior que %d ou menor que %d."),
        ...
        MensagensValidacaoVeiculo12(String descricao){
