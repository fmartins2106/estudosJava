...
public class Cliente36 {
    ...
    public Cliente36(String nome, String cpf) {
        setNome(nome);
        setCpf(cpf);
    }
    ...
    public static void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("^\\p{L}+( \\p{L}+)+$")){
            throw new IllegalArgumentException(MensagensValidacaoCliente36.NOME.getDescricao());
        }
    }
    ...
    public static void validacaoCpf(String cpf){
        if (cpf == null || cpf.isEmpty() || !cpf.matches("\\d{11}") || !isCpfValido(cpf)){
            throw new IllegalArgumentException(MensagensValidacaoCliente36.CPF.getDescricao());
        }
    }
    ...
    public enum MensagensValidacaoCliente36{
        NOME("Digite nome completo. Campo nome não pode ser vazio ou conter caracteres."),
        CPF("CPF inválido");

        private final String descricao;

        MensagensValidacaoCliente36(String descricao) {
