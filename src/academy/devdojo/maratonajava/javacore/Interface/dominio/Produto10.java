...
public class Produto10 implements Vendavel10{
    private String nome;
    private double preco;
    private int estoque;

    public Produto10(String nome, double preco, int estoque) {
        setNome(nome);
        setPreco(preco);
        setEstoque(estoque);
    }
    ...
    public static void validacaoNome(String nome){
        if (nome == null || nome.isEmpty() || !nome.matches("^[\\p{L}0-9]+( [\\p{L}0-9]+)*$")){
            throw new IllegalArgumentException(MensagensValidacaoProduto10.NOME.getDescricao());
        }
    }
    ...
    public static void validacaoPreco(double preco){
        if (preco < MENOR_PRECO){
            throw new IllegalArgumentException(MensagensValidacaoProduto10.PRECO.getDescricaoFormatada(MENOR_PRECO));
        }
    }

    public static final int MENOR_ESTOQUE = 0;

    private static void validacaoEstoque(int estoque){
        if (estoque < MENOR_ESTOQUE){
            throw new IllegalArgumentException(MensagensValidacaoProduto10.ESTOQUE.getDescricaoFormatada(MENOR_ESTOQUE));
        }
    }
    ...
    public enum MensagensValidacaoProduto10{
        ...
        MensagensValidacaoProduto10(String descricao) {
